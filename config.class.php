...
class PluginTrademarkConfig extends CommonDBTM {
   ...
   protected static function checkPicture($name, $input, $old, $width = 0, $height = 0, $max_size = 500) {
      ...
      if (isset($input[$blank]) && $input[$blank]) {
         ...
         if ($width || $height) {
            $image_info = getimagesize($picPath);
            $image_type = $image_info[2];

            if ($image_type === IMAGETYPE_GIF) {
               // Process GIF using GD
               $original_image = imagecreatefromgif($picPath);
               $resized_image = imagescale($original_image, $width, $height);
               imagegif($resized_image, $picResizedPath);
               imagedestroy($original_image);
               imagedestroy($resized_image);
            } else {
               // Process other image types using GD
